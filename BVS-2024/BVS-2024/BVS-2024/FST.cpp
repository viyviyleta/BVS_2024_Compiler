...
namespace FST {
	...
	// полный прогон конечного автомата по строке
	...
	void executeWordAndClear(std::vector<char>&word, std::vector<char> &str, std::vector<FSTAssigned> &FSTarray, 
		...
		str.clear();
		str = word;
		Lexema lexema = Lexema(word, line, col-1, 0xffffffff);
		for (int i = 0; i < FSTarray.size(); i++) {
			FSTarray[i].fst->string = str;
			if (execute(*FSTarray[i].fst)) {
				lexema = Lexema(word, line, col, 0xffffffff);
		...
		throw ERROR_THROW_IN(120, lexema.line, lexema.col); // не удалось разобрать
		word.clear();
	}

	// сравнение вектора с C-строкой
	...
	int VectorToInt(std::vector<char>word) {
		...
		// Конвертируем строку в int с помощью std::stringstream
		...
		for (int i = 0; i < lexems.size(); i++) {
			for (int j = 0; j < FSTarray.size(); j++) {
				...
				if (execute(*FSTarray[j].fst)) {
					...
					case LEX_TEXT:
					{
						if (FSTarray[j].iddatatype == IT::USINT) {
							iddatatype = IT::USINT;
						}
					...
					case LEX_ID:
					{
						...
						if (compareVectorToCString(lexems[i].word, "StrCmp")) {
							if (lextable.table[lextable.size - 1].lexema == LEX_FUNCTION) {
								throw ERROR_THROW_IN(127, lexems[i].line, lexems[i].col);
							}
							LTObj.idxTI = 2;
						...
						if (IT::IsId(idtable, (char*)scope.c_str()) == TI_NULLIDX) {
							...
							if (iddatatype != (IT::IDDATATYPE)0 && idtype != (IT::IDTYPE)0) {
								...
								if (iddatatype == IT::USINT) {
									IDTObj.value.vusint = 0;
								}
					...
					case LEX_LITERAL:
					{
						...
						if (lexems[i].word[0] == '\"') {
							...
							iddatatype = IT::USINT;
							ITObj.value.vusint = VectorToInt(lexems[i].word);
						}
	...
	void Analyze(In::IN in, LT::LexTable& lextable, IT::IdTable& idtable) {
		...
		std::vector<FSTAssigned> FSTarray = {
			...
			FSTAssigned(&lex_usint, IT::USINT, LEX_BYTE),							// 1
			...
			FSTAssigned(&lex_byteLiteral, IT::USINT, LEX_LITERAL),
