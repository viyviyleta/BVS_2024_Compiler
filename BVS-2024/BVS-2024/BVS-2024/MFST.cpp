...
namespace MFST {
	...
	char* Mfst::getDiagnosis(short n, char* buf, Log::LOG log) {
		if (n < MFST_DIAGN_NUMBER && diagnosis[n].lenta_position > 0 && diagnosis[n].lenta_position != 0) {
			Error::ERROR error = Error::geterrorin(greibach.getRule(diagnosis[n].nrule).idError, lex.lextable.table[diagnosis[n].lenta_position].sn, lex.lextable.table[diagnosis[n].lenta_position].cn);
			sprintf_s(buf, ERROR_MAXSIZE_MESSAGE, "Ошибка %d: строка %d,%s\n", error.id, lex.lextable.table[diagnosis[n].lenta_position].sn, error.message);
