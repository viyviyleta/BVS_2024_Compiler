...
#define TS(n) Rule::Chain::T(n)	
	...
	*/
	...
	Greibach greibach(NS('S'), TS('$'),
		...
		Rule(NS('C'), GRB_ERROR_SERIES + 1, // параметр функции (проверка добавляем ли еще что-то)
			2,
		...
		Rule(NS('N'), GRB_ERROR_SERIES + 3, // тело
			7,
			Rule::Chain(),
		...
		Rule(NS('E'), GRB_ERROR_SERIES + 4, //присваивания переменной какого-либо значения
			3,
			Rule::Chain(4, TS('i'), TS('('), NS('P'), TS(')')),
			Rule::Chain(1, TS('i')),
			Rule::Chain(1, TS('l'))
		),
		Rule(NS('O'), GRB_ERROR_SERIES + 5, // то что передаешь в display
			3,
			Rule::Chain(),
			Rule::Chain(1, TS('i')),
			Rule::Chain(1, TS('l'))
		),
		Rule(NS('P'), GRB_ERROR_SERIES + 6, //параметры функции
			5,
			Rule::Chain(),
		...
		Rule(NS('K'), GRB_ERROR_SERIES + 6, //добавления нескольких параметров
			3,
			Rule::Chain(),
			Rule::Chain(3, TS(','), TS('i'), NS('P')),
			Rule::Chain(3, TS(','), TS('l'), NS('P'))
		),
		Rule(NS('W'), GRB_ERROR_SERIES + 7, //скобки после if
			3,
			Rule::Chain(5, TS('i'), TS('('), NS('P'), TS(')'), NS('T')),
			Rule::Chain(2, TS('i'), NS('T')),
			Rule::Chain(2, TS('l'), NS('T'))
		),
		Rule(NS('T'), GRB_ERROR_SERIES + 7, //вспомогаетльный граф для того что может быть после скобок if
			4,
			Rule::Chain(),
	...
	char* Rule::getCRule(char* b, short nchain)
	{
		...
		strcat_s(b, sizeof(bchain) + 5, bchain);	//безопасно конкатенирует строки
		return b;
	}
	...
	short Rule::getNextChain(GRBALPHABET t, Chain& pchain, short j)
	{
		if (size == 0) {
			...
			return -2; // -2 как код для пустой цепочки
		}
